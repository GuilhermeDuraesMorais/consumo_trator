...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const popularSelectImplementos = (selectElementId) => {
        ...
        if (select) {
            // Opção padrão para "todos" nos filtros
            select.innerHTML = '<option value="">Todos os Implementos</option>';
            // Para o formulário de novo abastecimento, a primeira opção é "Selecione um Implemento"
    ...
    const loadPage = (page) => {
        try {
            ...
            switch (page) {
                ...
                case 'historico':
                    mainContent = `
                        ...
                        <div id="historico-vazio" class="text-center text-muted py-6 hidden">Nenhum abastecimento encontrado.</div>
                ...
                case 'graficos':
                    mainContent = `
                        ...
                        <div id="grafico-vazio" class="text-center text-muted py-6 hidden">Nenhum dado para os filtros.</div>`;
                    break;
            ...
            appContainer.innerHTML = `<div class="p-6 text-red-600">Erro: ${error.message}.</div>`;
        }
    };
    ...
    function initLogin() {
        ...
        if (!loginButton || !csInput || !nomeInput) return;
        ...
        loginButton.addEventListener('click', () => {
            ...
            if (cs && nome) {
                if (csInput.pattern && !new RegExp(`^${csInput.pattern}$`).test(cs)) {
                    alert('CS deve conter apenas números.'); csInput.focus(); return;
                }
                user = { cs, nome };
                localStorage.setItem('user', JSON.stringify(user));
                loadPage('abastecimento');
            } else {
                alert('Preencha CS e Nome.'); if (!cs) csInput.focus(); else if (!nome) nomeInput.focus();
            }
        });
    ...
    function initAbastecimento() {
        ...
        saveButton.addEventListener('click', () => {
            ...
            if (!data || !trator || isNaN(horimetroInicial) || isNaN(horimetroFinal) || !implemento || isNaN(litros) || litros <= 0 || horimetroInicial < 0 || horimetroFinal < 0) {
                alert('Preencha todos os campos corretamente. Litros e Horímetro devem ser positivos.'); return;
            }
            if (horimetroFinal <= horimetroInicial) {
                alert('Horímetro Final deve ser maior que Horímetro Inicial.'); horimetroFinalInput.focus(); return;
            }
            const currentUser = JSON.parse(localStorage.getItem('user'));
            if (!currentUser) { alert("Usuário não encontrado."); loadPage('login'); return; }
            ...
            implementoSelect.value = ""; // Reseta o select
            dataInput.value = new Date().toISOString().split('T')[0];
            tratorInput.focus();
        });
    }
    ...
    function initHistorico() {
        ...
        const exibirHistorico = (abastecimentosFiltrados) => {
            ...
            abastecimentosFiltrados.forEach(ab => {
                ...
                let implementoSelectHTML = `<select class="edit-input" data-field="implemento">`;
                ...
                if (editingIndex === originalIndex && isOwner) {
                    row.innerHTML = `
                        <td>${ab.cs}</td>
                        <td><input type="date" class="edit-input" value="${ab.data || ''}" data-field="data"></td>
                        <td><input type="text" class="edit-input" value="${ab.trator || ''}" data-field="trator"></td>
                        <td>${implementoSelectHTML}</td>
                        <td><input type="number" step="0.1" class="edit-input" value="${ab.horimetroInicial || ''}" data-field="horimetroInicial" min="0"></td>
                        <td><input type="number" step="0.1" class="edit-input" value="${ab.horimetroFinal || ''}" data-field="horimetroFinal" min="0"></td>
                        <td><input type="number" step="0.01" class="edit-input" value="${ab.litros || ''}" data-field="litros" min="0"></td>
        ...
        const attachTableButtonListeners = () => {
            tabelaHistorico.querySelectorAll('.editar-button, .excluir-button, .salvar-button, .cancelar-button').forEach(btn => {
                ...
                newBtn.addEventListener('click', (e) => {
                    ...
                    else if (action === 'excluir' && confirm('Excluir registro?')) { excluirAbastecimento(idx); }
            ...
            if (originalIndex >= 0 && originalIndex < abastecimentos.length && rowElement) {
                try {
                    ...
                    if (!newData.data || !newData.trator || !newData.implemento || isNaN(newData.horimetroInicial) || isNaN(newData.horimetroFinal) || isNaN(newData.litros) || newData.litros <= 0 || newData.horimetroInicial < 0 || newData.horimetroFinal < 0) {
                        alert('Erro: Preencha todos os campos corretamente.'); return;
                    }
                    if (newData.horimetroFinal <= newData.horimetroInicial) {
                        alert('Erro: Horímetro Final deve ser maior que Inicial.'); return;
                    }
        ...
        const createPDFBlob = (dataInicio, dataFim) => {
            try {
                ...
                if (filteredPDF.length === 0) return null;
            ...
            } catch (e) { console.error("Erro PDF:", e); return null; }
        };

        downloadPdfButton.addEventListener('click', () => {
            const di = pdfDataInicio.value, df = pdfDataFim.value;
            if (!di || !df) { alert("Selecione Data Início e Fim."); return; }
            if (new Date(df) < new Date(di)) { alert("Data Fim não pode ser anterior à Data Início."); return; }
            const blob = createPDFBlob(di, df);
            if (!blob) { alert("Não foi possível gerar PDF."); return; }
        ...
        sharePdfButton.addEventListener('click', async () => {
            ...
            if (navigator.share && navigator.canShare && navigator.canShare(shareData)) {
                try { await navigator.share(shareData); } catch (err) { if (err.name !== 'AbortError') alert('Erro ao compartilhar.');}
    ...
    function initGraficos() {
        ...
        const carregarDadosParaGrafico = () => {
            const currentUser = JSON.parse(localStorage.getItem('user'));
            if (!currentUser) {
                graficoVazioMsgEl.textContent = "Usuário não encontrado."; graficoVazioMsgEl.classList.remove('hidden');
            ...
            abastecimentos = abastecimentos.filter(ab => 
                ...
                (!tratorF || (ab.trator && ab.trator.toLowerCase().includes(tratorF))) &&
                (!implementoF || ab.implemento === implementoF) // Filtra por implemento se um for selecionado
            );
            ...
            graficoVazioMsgEl.textContent = "Nenhum dado para os filtros.";
            graficoVazioMsgEl.classList.toggle('hidden', labels.length > 0);
            graficoContainerEl.classList.toggle('hidden', labels.length === 0);

            if (labels.length > 0) renderizarGrafico(labels, dataConsumo, implementoSelecionadoTexto);
            else if (consumoChartInstance) consumoChartInstance.destroy();
        };
